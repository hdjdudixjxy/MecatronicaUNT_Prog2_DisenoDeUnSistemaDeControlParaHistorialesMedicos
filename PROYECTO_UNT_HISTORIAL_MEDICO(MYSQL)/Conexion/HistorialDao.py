...
def listarHistoria(idPersona):
    ...
    try:
        conexion.cursor.execute(sql)
        listaHistoria = list(conexion.cursor.fetchall())
        conexion.cerrarConexion()
    except:
        titulo = "LISTAR"
...
def seleccionarMedicoResponsable(idMedico):
    conexion = ConexionDB()
    listaMedicoResponsable = []
    
    sql = f"SELECT CONCAT(NombreMedico,' ',ApellidoMedico) AS Medico FROM Medicos WHERE idMedico = {idMedico}"

    conexion.cursor.execute(sql)
    listaMedicoResponsable = list(conexion.cursor.fetchall())
    conexion.cerrarConexion()


    return listaMedicoResponsable

def seleccionarDNIMedicoResponsable(idMedico):
    conexion = ConexionDB()
    listaDNIMedicoResponsable = []
    
    sql = f"SELECT DNI FROM Medicos WHERE idMedico = {idMedico}"

    conexion.cursor.execute(sql)
    listaDNIMedicoResponsable = list(conexion.cursor.fetchall())
    conexion.cerrarConexion()

    return listaDNIMedicoResponsable

def seleccionarIDMedicoResponsable(idHistorial, idPersonaHistorial):
    conexion = ConexionDB()
    listaIDMedicoResponsable = []
    
    sql = f"SELECT idMedico FROM HistoriaMedica WHERE idHistorial = {idHistorial} AND idPersona = {idPersonaHistorial}"
